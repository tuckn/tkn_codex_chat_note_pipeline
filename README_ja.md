...
生成・Raw取得コマンドは、結果の集計と保存済みレポートのパスを標準エラーへ簡潔に表示し、通常は標準出力へJSONを出しません。
`--full-output` を明示した場合のみ詳細JSONを表示します。`config show`などの参照コマンドはJSON出力を継続します。
`--dry-run`はレポートを保存しません。計画を残す場合は`--full-output`の標準出力を保存してください。
...
通常の出力は結果の集計1行と取得元ごとのレポート保存先です。`--full-output`を明示すると各会話を含む詳細JSONも表示します。
...
`generationEstimate`も表示します。集計・詳細は保存済みレポートにも残ります。
...
`inter_agent_communication_metadata`（`trigger_turn`など）は既知の制御情報としてRawに保持し、要約の根拠には含めません。
...
Rawと正規化済みイベントは変更しません。送信前にコンソールの色指定を除き、完全なHTMLから
本文を抽出し、同じ呼び出し・ターン・履歴で対応を確認できた重複を参照へ置き換えます。
大きなコード変更や、大量の一覧・検索結果は省略した行範囲を明示した抜粋にします。
先頭・末尾と、診断に関わる全行およびその前後を保持します。通常の会話文や形式を判定できない
文章は短縮しません。途中で切れたHTMLは本文抽出せず保持します。抜粋をファイル全体の説明とは扱いません。

API入力トークン上限を超えた分割だけを細分化します。他の分割を維持し、再分割しても
出典ID・順序・本文の位置を保ちます。保存済み生成指標に`inputPreparationVersion`、
`compactedCharacters`、`compactedEventCount`と既存の重複削減数を記録します。
`--dry-run`の見積もりにも削減文字数を含めます。

入力前処理とプロンプトのバージョンを生成・キャッシュの識別に含めます。古い途中結果は保持しますが、
互換性のないものは再利用しません。次回の通常の`pull`で古いノートが再生成対象になる場合があります。
編集済み・レビュー済みノートの保護は継続します。インストール時に過去のノートは再生成しません。
