...
日本語出力の英語表現WARNINGには、検出した表現・JSON内の場所（配列は0始まり）・短い抜粋を表示します。例：`supplied events at $.summaryItems[0].text: "The supplied events show completion."`。抜粋の改行・制御文字はJSON形式でエスケープし、一般的な認証情報の形式は伏せます。実行レポートの `generationMetrics.validationFailures`（`languageMatches` を含む）にも記録し、上限超過から再生成した場合は `repairFallbacks` に回数を残します。従来は失敗した下書きを保存していないため、過去のレポートから該当文を復元することはできません。
